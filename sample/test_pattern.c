    ...
    for (i = 0; i < ntests; ++i)
    {
        ...
        if (should_fail)
        {
            if (m != (-1))
            {
                ...
                rt_kprintf("[%lu/%lu]: pattern '%s' matched '%s' unexpectedly, matched %i chars. \n", (i+1), ntests, pattern, text, length);
                nfailed += 1;
            }
        }
        else
        {
            if (m == (-1))
            {
                rt_kprintf("\n");
                re_print(re_compile(pattern));
                rt_kprintf("[%lu/%lu]: pattern '%s' didn't match '%s' as expected. \n", (i+1), ntests, pattern, text);
                nfailed += 1;
            }
            else if (length != correctlen)
            {
                rt_kprintf("[%lu/%lu]: pattern '%s' matched '%i' chars of '%s'; expected '%i'. \n", (i+1), ntests, pattern, length, text, correctlen);
                nfailed += 1;
            }
        }
    }

    rt_kprintf("%lu/%lu tests succeeded.\n", ntests - nfailed, ntests);
